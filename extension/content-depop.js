// FlipStack AI — Depop Create Listing Autofill
// Runs on depop.com/products/create/*
// Checks FlipStack API for a pending post and fills the form.

(() => {
    if (window.__flipstack_depop_loaded) return;
    window.__flipstack_depop_loaded = true;

    const API = "http://localhost:5000";

    function log(msg) { console.log(`[FlipStack Depop] ${msg}`); }
    ...
    async function fillDepopForm(listing) {
        const results = { filled: [], skipped: [] };

        // --- DESCRIPTION (Depop uses description prominently) ---
        const descField = findField([
            'textarea[name="description"]',
            'textarea[aria-label*="description" i]',
            'textarea[placeholder*="description" i]',
            'textarea[placeholder*="Describe" i]',
        ]) || findByNearbyLabel(["description", "describe"]);
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.action === "fill" && message.listing) {
            fillDepopForm(message.listing).then(results => {
